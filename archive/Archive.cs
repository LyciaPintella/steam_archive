    ...
    class Archive
    {
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                Console.WriteLine("[D] Archive and delete the game files.");
                ...
                if (key == "D" || key == "K")
                {
                    ...
                    script.Add($"\"{zip}\" a -t7z -mx=7 \"{Path.Combine(Directory.GetCurrentDirectory(), "backups", archiveName)}.7z\" \"{string.Join("\" \"", paths.ToArray())}\"");
